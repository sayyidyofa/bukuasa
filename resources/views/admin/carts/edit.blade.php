    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.carts.update", [$cart->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('faktur') ? 'is-invalid' : '' }}" name="faktur_id" id="faktur_id" required>
                    @foreach($fakturs as $id => $faktur)
                        <option value="{{ $id }}" {{ (old('faktur_id') ? old('faktur_id') : $cart->faktur->id ?? '') == $id ? 'selected' : '' }}>{{ $faktur }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $cart->product->id ?? '') == $id ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('weight_kg') ? 'is-invalid' : '' }}" type="number" name="weight_kg" id="weight_kg" value="{{ old('weight_kg', $cart->weight_kg) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_unit') ? 'is-invalid' : '' }}" type="number" name="amount_unit" id="amount_unit" value="{{ old('amount_unit', $cart->amount_unit) }}" step="1">
