    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pembayarans.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Pembayaran::HOLDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('holder') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="holder_{{ $key }}" name="holder" value="{{ $key }}" {{ old('holder', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nth_payment') ? 'is-invalid' : '' }}" type="number" name="nth_payment" id="nth_payment" value="{{ old('nth_payment', '') }}" step="1">
