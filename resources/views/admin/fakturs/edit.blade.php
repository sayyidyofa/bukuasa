...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.faktur.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fakturs.update", [$faktur->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pelanggan') ? 'is-invalid' : '' }}" name="pelanggan_id" id="pelanggan_id" required>
                    @foreach($pelanggans as $id => $pelanggan)
                        <option value="{{ $id }}" {{ (old('pelanggan_id') ? old('pelanggan_id') : $faktur->pelanggan->id ?? '') == $id ? 'selected' : '' }}>{{ $pelanggan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_faktur') ? 'is-invalid' : '' }}" type="number" name="no_faktur" id="no_faktur" value="{{ old('no_faktur', $faktur->no_faktur) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_faktur') ? 'is-invalid' : '' }}" type="text" name="tgl_faktur" id="tgl_faktur" value="{{ old('tgl_faktur', $faktur->tgl_faktur) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tagihan') ? 'is-invalid' : '' }}" type="number" name="tagihan" id="tagihan" value="{{ old('tagihan', $faktur->tagihan) }}" step="0.01" required>
