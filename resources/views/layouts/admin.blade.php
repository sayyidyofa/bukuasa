...
<head>
    ...
    <link href="https://unpkg.com/@coreui/coreui@3.2/dist/css/coreui.min.css" rel="stylesheet" type="text/css"/>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css" rel="stylesheet" type="text/css"/>
...
<body class="c-app">
    ...
    <script>
        $('#toWeeklySalaryForm').on('click', (e) => {
            Swal.fire({
                title: 'Pilih Waktu',
                showCancelButton: true,
                showConfirmButton: false,
                html: '<input name="weeklySalaryDateRange">',
                didOpen: () => {
                    $('input[name="weeklySalaryDateRange"]').daterangepicker({
                        opens: 'left',
                    }, (start, end, _) => {
                        window.location.href = `{{ route('admin.weeklySalaryForm') }}?start_date=${start.format('YYYY-MM-DD')}&end_date=${end.format('YYYY-MM-DD')}&_token={{ csrf_token() }}`;
                    })
                }
            })
        });
